...
tensorboard_path = '/home/featurize/cikm22/exp/tensorboard_logs/doublegann4_bn/score'
config_path = './config/double_gnn2.yaml'
client_model_save_path = '/home/featurize/cikm22/result/doublegann4_bn/'
client_tensorboard_path = '/home/featurize/cikm22/exp/tensorboard_logs/doublegann4_bn/'
...
if __name__ == '__main__':
    ...
    for i in range(1,14):
        ...
        # for i in range(3): # 共用ggnn的global mlp
        #     client.model.ggnn.convs[i].nn.linears = global_gnn.convs[i].nn.linears
        # client.model.gbn = global_bn # 共用一个全局BN
        # 初始化相同的参数
        tmp ={}
    ...
    global_gnn_opt = torch.optim.Adamax(global_gnn.parameters(),lr=0.01)
    # global_bn_opt = torch.optim.Adamax(global_bn.parameters(),lr=0.01)

    for round in range(ROUND):

        # 单独训练一轮
        for j, client in enumerate(all_client):
            client.update(encoder=True,gnn=False,ggnn=False,ggnn_bn=True,pgnn=True,linear=True,glinear=False,plinear=False,clf=True,gtr=False,stage='is',kd=False)
        ...
        # gnn 和 gnnc训练
        for j, client in enumerate(all_client):
            client.update(encoder=False,gnn=False,ggnn=False,ggnn_bn=True,pgnn=True,linear=False,glinear=False,plinear=False,clf=False,gtr=False,stage='ci',kd=False)
        ...
        for i, client in enumerate(all_client):
            client.update(encoder=True,gnn=False,ggnn=False,ggnn_bn=True,pgnn=False,linear=True,glinear=False,plinear=False,clf=True,gtr=False,stage='ft',kd=False)
