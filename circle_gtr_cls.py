...
# base_error = [0.263789,0.289617,0.355404,0.396825,0.302378]
...
tensorboard_path = '/home/featurize/cikm22/exp/tensorboard_logs/only_cls_try_fedap/score'
config_path = './config/config.yaml'
client_model_save_path = '/home/featurize/cikm22/result/only_cls_try_fed_ap/'
client_tensorboard_path = '/home/featurize/cikm22/exp/tensorboard_logs/only_cls_try_fed_ap/'
...
if __name__ == '__main__':
    ...
    # 加载所有的数据
    all_dl = load_client_data(config.data_sp,[1,2,3,4,5,6,7,8])
    ...
    fedlayer = []
    for name,value in client.model.named_parameters():
        if 'encoder' not in name and 'norm' not in name and '.lin.' not in name and 'ln' not in name:
            fedlayer.append(name)
    print(fedlayer)
    ...
    for i in range(400):
        ...
        for j, client in enumerate(all_client):
            client.update(encoder=True,gnn=True,gtr=False,linear=False,clf=False,stage='is',kd=False,prox=False)
        
        for j, client in enumerate(all_client):
            client.update(encoder=False,gnn=True,gtr=True,linear=False,clf=True,stage='is',kd=False,prox=False)
