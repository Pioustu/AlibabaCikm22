...
tensorboard_path = '/home/featurize/cikm22/exp/tensorboard_logs/doublegann2/score'
config_path = './config/double_gnn2.yaml'
client_model_save_path = '/home/featurize/cikm22/result/doublegann2/'
client_model_save_path_ft = '/home/featurize/cikm22/result/doublegann2/'
client_tensorboard_path = '/home/featurize/cikm22/exp/tensorboard_logs/doublegann2/'
...
if __name__ == '__main__':
    ...
    for round in range(ROUND):
        ...
        for i, client in enumerate(all_client):
            all_gnn_c.append(copy.deepcopy(client.model.gnn.state_dict()))
        
        # 聚合算法
        new_all_gnn_c = FedBNGnn2(all_gnn_c)

        # print(new_all_gnn_c[0].keys())
        # 将聚聚合后的gnn_c装回去，装到gnn_c
        for i, client in enumerate(all_client):
            client.model.gnn_c.load_state_dict(new_all_gnn_c[i])

        # 单独训练一轮，gnn_c作为全局gnn不进行更新
        val_error = []
        best_val_error = []
        for i, client in enumerate(all_client):
        ...
        for i, client in enumerate(all_client):
            client.update(stage='ft',gnn=False,gnn_c=False)
